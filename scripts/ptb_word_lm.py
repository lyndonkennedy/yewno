...
# ptb_word_lm.py
# 
# extension of script of same name from TensorFlow distribution.
# altered to allow iterative feedback in prediction phase to generate strings of terms
#
# ==============================================================================
...
flags.DEFINE_string("train",None,"do training");
flags.DEFINE_string("length",100,"do training");
...
#-------------
def predict_words(session, m, instr, length, words, word_to_id, eval_op, verbose=False):
	costs = 0.0
	iters = 0
	state = m.initial_state.eval()
	...
	# feed input words through RNN
	y[0,0] = word_to_id['<unk>'];
	inwords = instr.strip().split();
	for word in inwords:
	...
	# iteratively feed predicted words back into RNN
	x[0,0] = np.argmax(logits);
	for i in range(length):
		cost,state,logits, _ = session.run([m.cost,m.final_state,m.logits,eval_op],
									{m.input_data: x, 
									m.targets: y,
									m.initial_state: state})
		#hack to force the method to choose a word other than "<unk>"
		logits[0][0] = -1000000;							
		x[0,0] = np.argmax(logits);
		outstr += words[np.argmax(logits)]+" ";
